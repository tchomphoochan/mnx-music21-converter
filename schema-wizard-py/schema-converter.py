    ...
    def work(self, c: Union[JsonClass, "JsonAlias"]):
        ...
        try:
            ...
            for name, type in c.optionalFields:
                ...
                self.line(
                    f"{k}: {oldtype} = json_field(['{name}'], default_factory={type})"
                )
            if c.optionalFields == []:
                self.line("pass")
        finally:
...
def parse_property(
    key: str, property: dict
) -> Tuple[str, List[Union[JsonClass, JsonAlias]]]:
    match property:
        case {"type": "integer"}:
            n = nice_name(key)
        ...
        case {
            ...
            **rest,
            ...
            ), "We only handle additionalProperties = False"
